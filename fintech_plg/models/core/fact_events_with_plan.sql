...
-- Falls back to current plan from dim_users if snapshot doesn't have a match (e.g., events before first snapshot)
select
  f.event_id,
  f.user_id,
  f.event_date,
  f.event_type,
  f.amount,
  coalesce(s.plan_type, d.plan_type) as plan_type
